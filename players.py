...
class BluePlayer():
    ...
    def draw(self, WIN):
        WIN.blit(self.BLUE_SPACESHIP, (self.x, self.y))
    ...
    def blue_movement(self, keys_pressed):
        if keys_pressed[pygame.K_a] and self.x - SHIP_VEL > 0:  # LEFT
            self.x -= SHIP_VEL
        if keys_pressed[pygame.K_d] and self.x + SHIP_VEL + self.width < BORDER.x:  # RIGHT
            self.x += SHIP_VEL
        if keys_pressed[pygame.K_w] and self.y - SHIP_VEL > 0:  # UP
            self.y -= SHIP_VEL
        if keys_pressed[pygame.K_s] and self.y + SHIP_VEL + self.width < HEIGHT:  # DOWN
            self.y += SHIP_VEL
...
class RedPlayer():
    ...
    def red_movement(self, keys_pressed):
        if keys_pressed[pygame.K_LEFT] and self.x - SHIP_VEL > BORDER.x:  # LEFT
            self.x -= SHIP_VEL
        if keys_pressed[pygame.K_RIGHT] and self.x + SHIP_VEL + self.width < WIDTH:  # RIGHT
            self.x += SHIP_VEL
        if keys_pressed[pygame.K_UP] and self.y - SHIP_VEL > 0:  # UP
            self.y -= SHIP_VEL
        if keys_pressed[pygame.K_DOWN] and self.y + SHIP_VEL + self.width < HEIGHT:  # DOWN
            self.y += SHIP_VEL
